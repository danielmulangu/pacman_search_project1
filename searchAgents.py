...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        distance_from_goal = [] # Calculate all distances from goals that aren't visited

        for index,item in enumerate(state[1]):
            if item == 0: # Not visited corner
                
                distance_from_goal.append(manhattanDistance(state[0],corners[index]))

        # Worst case. Pick higher distance #
        return max(distance_from_goal)
...
def foodHeuristic(state, problem):
    ...
    problem.heuristicInfo['wall_counter'] = problem.walls.count()
    if problem.isGoalState(state):
        return 0
    dist= [] #Calculate the distance between the position and all the food
    flag=0
    
    for obj in food_list:
        dist.append(mazeDistance(position,obj,problem.startingGameState))
        if flag== 4 or problem.heuristicInfo['wall_counter'] > 20:
            break
        flag+=1
